...
set.seed(1234) # if you want reproducible results
...
CV$TestSize <- sum(CV$which==1)

# Pruning levels
prune = seq(from=0, to=0.05, length.out=500); # very problem dependent what pruning level should be set to
...
mytree <- rpart(fmla, data=Xdatframe_train,control=rpart.control(minsplit=2, minbucket=1, cp=0), parms=list(split='gini'), method="class")
