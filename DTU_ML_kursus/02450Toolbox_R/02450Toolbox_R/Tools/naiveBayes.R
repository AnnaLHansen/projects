...
########### fit paramaters
  if(any(normalFeats)){
    for(iclass in 1:nClasses){
      ...
      sigma <- apply(X[currentClassIndices,normalFeats,drop=FALSE], 2, fun <- function(x){sd(x, na.rm=TRUE)}) # column sd's
  ...
  if(any(mvmnFeats)){
    possibleFeatVals <- apply(X=X[,mvmnFeats,drop=FALSE], MARGIN=2, FUN=function(x){list(sort(unique(x)))}) # find the observed values for each feature
    for(iclass in 1:nClasses){
      currentClassIndices <- y==classLevels[iclass]
